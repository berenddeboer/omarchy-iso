...
def _install_disk(ctx: InstallContext) -> str | None:
    ...
    if ctx.storage.get("wipe"):
        return ctx.storage.get("disk")
    return None
...
def prepare_install_target(ctx: InstallContext) -> None:
    if ctx.is_zfs:
        from . import zfs_storage
        zfs_storage.prepare(ctx)
...
def _seed_zfs_user_home(ctx: InstallContext) -> None:
    """Archinstall skips /etc/skel when the mounted ZFS home already exists."""
    from . import zfs_storage

    zfs_storage.validate_mounts(ctx)
    source = ctx.target / "etc/skel"
    destination = ctx.target / "home" / ctx.username
    subprocess.run(["cp", "-a", f"{source}/.", str(destination)], check=True)
    subprocess.run(
        ["arch-chroot", str(ctx.target), "chown", "-R", f"{ctx.username}:{ctx.username}", f"/home/{ctx.username}"],
        check=True,
    )
    shell_config = destination / ".config/omarchy/shell.json"
    if not shell_config.exists():
        raise RuntimeError(f"ZFS home was not seeded from /etc/skel: {shell_config} missing")
...
def _runtime_package_list(ctx: InstallContext) -> list[str]:
    ...
    for raw in base_pkgs_file.read_text().splitlines():
        ...
        if s not in already_installed and s not in pkgs:
            pkgs.append(s)
    if ctx.is_zfs:
        kernel = ctx.storage.get("kernel") or (ctx.user_configuration.get("kernels") or ["linux"])[0]
        headers = f"{kernel}-headers"
        for package in ("dkms", headers, "zfs-utils", "zfs-dkms"):
...
def _write_pre_mounted_fstab(ctx: InstallContext) -> None:
    ...
    if ctx.is_zfs:
        lines = [
            ...
            "# ZFS datasets use native mountpoints; only the ESP belongs here.",
...
def _build_pre_mounted_cmdline(ctx: InstallContext, btrfs_uuid: str | None = None) -> str:
    storage = _storage_intent(ctx)
    if ctx.is_zfs:
        return f"root=ZFS={storage['root_dataset']} zfs_boot_only=1 zswap.enabled=0 rw"
    if storage.get("luks_uuid"):
    ...
    if not btrfs_uuid:
        raise RuntimeError("Btrfs UUID missing while building kernel command line")
    return (
...
def _write_pre_mounted_limine_defaults(ctx: InstallContext) -> None:
    boot = _boot_intent(ctx)
    if ctx.is_zfs:
        cmdline = _build_pre_mounted_cmdline(ctx)
    else:
...
def configure_hibernation(ctx: InstallContext) -> None:
    ...
    """
    if ctx.is_zfs:
        _debug_log(ctx, "skipping hibernation: ZFS root has no supported resume layout")
        return
...
def run_system_finalizer(ctx: InstallContext) -> None:
    if ctx.is_zfs:
        key = Path("/usr/share/omarchy-iso/archzfs.gpg")
        if not key.exists():
            raise RuntimeError("ArchZFS signing key is missing from the ISO")
        target_key = ctx.target / "root/archzfs.gpg"
        target_key.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(key, target_key)
        subprocess.run(["arch-chroot", str(ctx.target), "pacman-key", "--add", "/root/archzfs.gpg"], check=True)
        subprocess.run([
            "arch-chroot", str(ctx.target), "pacman-key", "--lsign-key",
...
def configure_login(ctx: InstallContext) -> None:
    ...
    if ctx.boot_requires_passphrase and not ctx.defer_provisioning:
...
def validate_boot(ctx: InstallContext) -> None:
    ...
    if ctx.has_luks_root and "cryptdevice=" not in limine_conf_text:
        raise RuntimeError(f"Encrypted install but {limine_conf} has no cryptdevice=")
    ...
    if ctx.is_zfs:
        from . import zfs_storage
        zfs_storage.validate_target(ctx, limine_conf_text)
    elif ctx.is_protected:
...
def cleanup_storage_state(ctx: InstallContext, *, success: bool) -> None:
    if ctx.is_zfs:
        from . import zfs_storage
        zfs_storage.cleanup(ctx)
    elif not success:
